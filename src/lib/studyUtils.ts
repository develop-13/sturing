...
export async function getUserInterestStudies(userMatchingInfo: NewTMatching) {
  ...
  const usrInterestStudies = await Study.find({ categories: firstInterest })
    .sort({ score: -1 }) // 평점(score) 기준으로 내림차순 정렬
    .limit(5) // 상위 5개만 가져오기
    .exec();

  console.log(`usrInterestStudies=${usrInterestStudies}`);

  return usrInterestStudies; // 평점순 상위 5개의 스터디 반환
}
